...
#ifndef __JPEGDEC__
#define __JPEGDEC__
#if defined( __MACH__ ) || defined( __LINUX__ ) || defined( __MCUXPRESSO ) || defined( ESP_PLATFORM ) || defined(_WIN64) || defined(STM32H723xx)
#include "stm32h7xx.h"
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
#endif
#if defined(__CC_ARM)
/* ARM Compiler 5缺少GCC的字节交换内建函数，使用等价的定宽实现。 */
static __inline uint16_t jpegdec_bswap16(uint16_t value)
{
    return (uint16_t)((value << 8) | (value >> 8));
}

static __inline uint32_t jpegdec_bswap32(uint32_t value)
{
...
static __inline uint64_t jpegdec_bswap64(uint64_t value)
{
    return ((uint64_t)jpegdec_bswap32((uint32_t)value) << 32) |
           jpegdec_bswap32((uint32_t)(value >> 32));
}

#define __builtin_bswap16 jpegdec_bswap16
#define __builtin_bswap32 jpegdec_bswap32
#define __builtin_bswap64 jpegdec_bswap64
#endif
#ifndef PROGMEM
